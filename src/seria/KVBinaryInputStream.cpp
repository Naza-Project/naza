...
namespace {
	...
	size_t read_varint(common::IInputStream &s) 
	{
		...
		auto size_mask = b & PORTABLE_RAW_SIZE_MARK_MASK;
		size_t bytes_left = 0;
	...
	JsonValue load_entry(common::IInputStream &stream) 
	{
		auto type = read_pod<uint8_t>(stream);
