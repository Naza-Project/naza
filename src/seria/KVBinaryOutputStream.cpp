...
void KVBinaryOutputStream::begin_array(size_t &size, bool fixed_size) 
{
	...
	{
		auto &s = stream();
		unsigned char c =			BIN_KV_SERIALIZE_FLAG_ARRAY | BIN_KV_SERIALIZE_TYPE_STRING;  // we do not care which type is empty array
		s.write(&c, 1);
...
{
	auto valid_array = m_stack.back().state == State::Array;
	m_stack.pop_back();
