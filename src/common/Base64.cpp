	...
	namespace base64
	{
		...
		bool decode(const std::string &in, BinaryArray *ret)
		{
			...
			for (size_t  i = 0; i < encoded_size; i += 4)
			{
				...
				const uint8_t b4_0 =	(static_cast<uint8_t>(in[i + 0]) <= 'z') ? from_base64[static_cast<uint8_t>(in[i + 0])] : 0xff;
				const uint8_t b4_1 =	(i + 1 < N && static_cast<uint8_t>(in[i + 1]) <= 'z') ? from_base64[static_cast<uint8_t>(in[i + 1])] : 0xff;
				const uint8_t b4_2 =	(i + 2 < N && static_cast<uint8_t>(in[i + 2]) <= 'z') ? from_base64[static_cast<uint8_t>(in[i + 2])] : 0xff;
				const uint8_t b4_3 =	(i + 3 < N && static_cast<uint8_t>(in[i + 3]) <= 'z') ? from_base64[static_cast<uint8_t>(in[i + 3])] : 0xff;
