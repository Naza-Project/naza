...
{
	if (size != other.size)		return int(size) - int(other.size);
...
{
	auto op = find_option(SView(key));
	if (!op)		return typeid(std::nullptr_t);
	if (op->values.size() != 1)
...
const std::vector<const char *> &CommandLine::get_positional(const char *deprecation_text)
{
	...
	if (deprecation_text)	printf("Positional command line options are deprecated. %s\n", deprecation_text);
	return positional;
}
...
bool CommandLine::should_quit(const char *help_text, const char *version_text)
{
	...
	if (version_text && (v1 || v2))
	{  // in case both specified
		printf("%s\n", version_text);
