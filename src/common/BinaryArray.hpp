...
namespace common
{
	...
	public:
		...
		BinaryArrayImpl() = default;
		~BinaryArrayImpl() { delete[] m_data; }
		explicit BinaryArrayImpl(size_t si) { alloc(si); }
		explicit BinaryArrayImpl(size_t si, value_type va) {	alloc(si); 	memset(m_data, va, si);	}
		explicit BinaryArrayImpl(std::initializer_list<value_type> li) {	alloc(li.size()); 	memmove(m_data, li.begin(), li.size());		}
		explicit BinaryArrayImpl(const value_type *be, const value_type *en) {		alloc(en - be);	good_memmove(m_data, be, en - be);		}
		explicit BinaryArrayImpl(const char *be, const char *en) {		alloc(en - be);		good_memmove(m_data, be, en - be);		}
		BinaryArrayImpl(const BinaryArrayImpl &other) {		alloc(other.size());		good_memmove(m_data, other.m_data, m_size);	}
		...
		ptrdiff_t compare(const BinaryArrayImpl &other) const 
		{
			...
			return diff != 0 ? static_cast<ptrdiff_t>(diff)	: static_cast<ptrdiff_t>(m_size) - static_cast<ptrdiff_t>(other.m_size);
		}
	...
	private:
		...
		void alloc(size_t si) { m_size = m_reserved = si;	m_data = new value_type[m_reserved];	}
	...
	using BinaryArray = BinaryArrayImpl;

	template<class It>
	inline BinaryArray::iterator append(BinaryArray &ba, It be, It en) {	return ba.insert(ba.end(), be, en);		}
	inline BinaryArray::iterator append(BinaryArray &ba, size_t add, BinaryArray::value_type va) {	return ba.insert(ba.end(), add, va);	}
