...
namespace common 
{
	namespace base58
	{
		namespace
		{
			...
			struct decoded_block_sizes
			{
				decoded_block_sizes() 
				{
					...
					for (auto i = 0; i <= full_block_size; ++i) 
					{
			...
			bool decode_block(const char *block, size_t size, uint8_t *res) 
			{
				...
				{
					auto digit = reverse_alphabet::instance(block[i]);
		...
		bool decode(const std::string &enc, BinaryArray *data) 
		{
			...
			if (0 < last_block_size) 
			{
				if (!decode_block(enc.data() + full_block_count * full_encoded_block_size, last_block_size, &(*data)[full_block_count * full_block_size]))
					return false;
			}
			return true;
		}
