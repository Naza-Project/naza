...
namespace common 
{
	...
	bool from_hex(const std::string &text, void *data, size_t buffer_size,	size_t &size);  // Assigns values of hex 'text' to buffer 'data' up to 'buffer_size', assigns actual data size to
	...
	inline bool split_string_helper(	const std::string &str, size_t pos, const std::string &separator, std::string &head, Parts &... parts)
	{
