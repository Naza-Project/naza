...
size_t StringInputStream::copy_to(IOutputStream &out, size_t max_count)
{
	...
	while (true) 
	{
		auto rc = std::min(in->size() - in_position, max_count);
		if (rc == 0)
			break;
		auto count = out.write_some(in->data() + in_position, rc);
		in_position += count;
		max_count -= count;
		total_count += count;
		if (count == 0)
...
{
	auto rc = std::min(size, read_count());
...
{
	auto rc = std::min(size, write_count());
...
void CircularBuffer::copy_from(IInputStream &in)
{
	while (true) 
	{
		...
		if (wc == 0)
			break;
		auto count = in.read_some(write_ptr(), wc);
		did_write(count);
		if (count == 0)
			break;
	}
...
size_t CircularBuffer::copy_to(IOutputStream &out, size_t max_count)
{
	...
	while (true) 
	{
		auto rc = std::min(read_count(), max_count);
		if (rc == 0)
			break;
		auto count = out.write_some(read_ptr(), rc);
		did_read(count);
		max_count -= count;
		total_count += count;
		if (count == 0)
