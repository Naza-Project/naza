...
namespace nazacoin {
	...
	class UnspentSelector {
		...
		using Unspents = std::vector<api::Output>;

		using HaveCoins = std::map<size_t, std::map<size_t, std::vector<api::Output>>>;


		using DustCoins = std::map<Amount, std::vector<api::Output>>;
		Unspents m_unspents;
