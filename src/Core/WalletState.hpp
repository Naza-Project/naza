...
class WalletState : public WalletStateBasic {
	class DeltaState : public IWalletState {
		using Unspents = std::map<PublicKey, std::vector<api::Output>>;
		Unspents m_unspents;
		std::map<KeyImage, int> m_used_keyimages;  
		std::map<Hash, std::pair<TransactionPrefix, api::Transaction>> m_transactions;

	public:
		explicit DeltaState() = default;
		void clear();
		...
		void undo_transaction(const Hash &tid); 

		virtual Amount add_incoming_output(const api::Output &) override; 
		virtual Amount add_incoming_keyimage(Height, const KeyImage &) override;
		virtual void add_transaction(Height, const Hash &tid, const TransactionPrefix &tx, const api::Transaction &ptx) override;
	};
...
protected:
	...
	bool parse_raw_transaction(api::Transaction *ptx, std::vector<api::Transfer> *input_transfers, Amount *output_amount, const PreparedWalletTransaction &pwtx, Hash tid,  const std::vector<uint32_t> &global_indices, Height block_heights) const;

	bool redo_transaction(const PreparedWalletTransaction &pwtx, const std::vector<uint32_t> &global_indices,  IWalletState *delta_state, bool is_base, Hash tid, Height block_height,Hash bid, Timestamp tx_timestamp);
...
private:
	...
	using PaymentQueue = boost::multi_index_container<QueueEntry, boost::multi_index::indexed_by<boost::multi_index::ordered_unique<boost::multi_index::tag<by_hash>,
