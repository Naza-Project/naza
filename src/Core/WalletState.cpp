...
void WalletState::DeltaState::add_transaction(Height, const Hash &tid, const TransactionPrefix &tx, const api::Transaction &ptx)
{
	invariant(m_transactions.insert(std::make_pair(tid, std::make_pair(tx, ptx))).second, "transaction already exists. Invariant dead");
...
void WalletState::wallet_addresses_updated() {
	...
	try {
		...
		while (!empty_chain() && get_tip().timestamp + m_currency.get_block_future_time_limit(get_tip_height() + 1) >= undo_timestamp) {  
...
std::vector<WalletRecord> WalletState::generate_new_addresses(const std::vector<SecretKey> &sks, Timestamp ct, Timestamp now) {
...
void WalletState::process_payment_queue_send_error(Hash hash, const api::nazad::SendTransaction::Error &error) {
	if (error.code == api::nazad::SendTransaction::OUTPUT_ALREADY_SPENT ||
		...
		if (get_tip_height() > error.conflict_height + m_currency.expected_blocks_per_day()) {
			...
			platform::remove_file(m_wallet.get_payment_queue_folder());  
			m_pq_version += 1;
		}
	}
}
...
void WalletState::add_transaction_to_mempool(Hash tid, TransactionPrefix &&tx, bool from_pq) {
	...
	if (!redo_transaction(pwtx, global_indices, &m_memory_state, false, tid, get_tip_height() + 1, Hash{}, get_tip().timestamp)) {
	} 
}
...
bool WalletState::sync_with_blockchain(api::nazad::SyncMemPool::Response &resp) {
	...
	for (size_t i = 0; i != resp.added_raw_transactions.size(); ++i) {
		...
		Hash tid = resp.added_transactions.at(i).hash;  
...
bool WalletState::redo_block(const api::BlockHeader &header, const PreparedWalletBlock &pb, const BlockChainState::BlockGlobalIndices &global_indices, Height height) {
	invariant(height == get_tip_height() + 1, "Redo of incorrect block height");
	if (global_indices.size() != pb.transactions.size() + 1)
		return false;                         
	Hash base_hash = pb.base_transaction_hash; 
	if (!redo_transaction(pb.base_transaction, global_indices[0], this, true, base_hash, get_tip_height() + 1, header.hash, pb.header.timestamp)) {
	}  // Just ignore
	for (size_t tx_index = 0; tx_index != pb.transactions.size(); ++tx_index) {
		...
		if (m_pool_hashes.erase(tid) != 0)
			remove_transaction_from_mempool(tid, false);
		m_memory_state.undo_transaction(tid);
		if (!redo_transaction(pb.transactions.at(tx_index), global_indices.at(tx_index + 1), this, false, tid, get_tip_height() + 1, header.hash, pb.header.timestamp)) {
		} 
	}
...
bool WalletState::parse_raw_transaction(api::Transaction *ptx, std::vector<api::Transfer> *input_transfers, Amount *unrecognized_inputs_amount, const PreparedWalletTransaction &pwtx, Hash tid, const std::vector<uint32_t> &global_indices, Height block_height) const {
	if (global_indices.size() != pwtx.tx.outputs.size()) 
		return false;  
	...
	std::map<AccountPublicAddress, api::Transfer> transfer_map_outputs[2];
	for (const auto &output : tx.outputs) {
...
bool WalletState::parse_raw_transaction(api::Transaction &ptx, const TransactionPrefix &tx, Hash tid) const {
	...
	parse_raw_transaction(&ptx, &input_transfers, &unrecognized_inputs_amount, pwtx, tid, global_indices, get_tip_height());
...
bool WalletState::redo_transaction(const PreparedWalletTransaction &pwtx, const std::vector<uint32_t> &global_indices, IWalletState*delta_state, bool is_base, Hash tid, Height block_height, Hash bid, Timestamp tx_timestamp) {
	api::Transaction ptx;
	Amount unrecognized_inputs_amount = 0;
	std::vector<api::Transfer> input_transfers;
	if (!parse_raw_transaction(&ptx, &input_transfers, &unrecognized_inputs_amount, pwtx, tid, global_indices, block_height))
		return false;  
	if (is_base)
		ptx.fee = 0;
	ptx.block_hash = bid;
