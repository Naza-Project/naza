...
namespace nazacoin {
	...
	class Wallet {
		...
		HistoryKey m_history_key;   
		Hash m_history_filename_seed; 
	...
	public:
		...
		std::vector<WalletRecord> generate_new_addresses(const std::vector<SecretKey> &sks, Timestamp ct, Timestamp now, bool *rescan_from_ct);  // set secret_key to SecretKey{} to generate
		...
		void on_first_output_found(Timestamp ts); 

		using History = std::set<AccountPublicAddress>;
