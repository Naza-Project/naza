...
namespace nazacoin {
	...
	public:
		...
		virtual void add_transaction(Height block_height, const Hash &tid, const TransactionPrefix &tx, const api::Transaction &ptx) = 0;
	};
	...
	public:
		...
		bool api_add_unspent(std::vector<api::Output> *result, Amount *total_amount, const std::string &address, Height height, Amount max_amount = std::numeric_limits<Amount>::max()) const;
		std::vector<api::Block> api_get_transfers(const std::string &address, Height *from_height, Height *to_height, bool forward, uint32_t desired_tx_count = std::numeric_limits<uint32_t>::max()) const;
		virtual std::vector<api::Output> api_get_locked_or_unconfirmed_unspent(const std::string &address, Height height) const;
		virtual api::Balance get_balance(const std::string &address, Height height) const;
		std::map<std::pair<Amount, uint32_t>, api::Output> api_get_unlocked_outputs(const std::string &address, Height from_height, Height to_height = std::numeric_limits<Height>::max()) const;
	...
	private:
		...
		using UndoMap = std::map<std::string, UndoValue>;
		...
		void read_unlock_index(std::map<std::pair<Amount, uint32_t>, api::Output> *add, const std::string &index_prefix, const std::string &address, uint32_t begin, uint32_t end) const;
