...
void P2PClientBasic::send_timed_sync() {
	...
	auto msg = LevinProtocol::send_message(COMMAND_TIMED_SYNC::ID, LevinProtocol::encode(req), true);
...
void P2PClientBasic::on_connect() {
	...
	auto msg = LevinProtocol::send_message(COMMAND_HANDSHAKE::ID, LevinProtocol::encode(req), true);
...
void P2PClientBasic::msg_handshake(COMMAND_HANDSHAKE::request &&req) {
	...
	auto raw_msg = LevinProtocol::send_reply(COMMAND_HANDSHAKE::ID, LevinProtocol::encode(msg), 0);
...
void P2PClientBasic::msg_ping(COMMAND_PING::request &&req) {
	...
	auto raw_msg = LevinProtocol::send_reply(COMMAND_PING::ID, LevinProtocol::encode(msg), 0);
	send(std::move(raw_msg));
	send_shutdown();
...
void P2PClientBasic::msg_timed_sync(COMMAND_TIMED_SYNC::request &&req) {
	...
	auto raw_msg = LevinProtocol::send_reply(COMMAND_TIMED_SYNC::ID, LevinProtocol::encode(msg), 0);
	send(std::move(raw_msg));
